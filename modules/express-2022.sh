...
# Lesson exceptions
if [[ "${LESSON}" == "14" ]]; then
    # Restrict videos to be pretty small
    VIDEO_MAX_SIZE=24000000
fi
...
if [[ "${LESSON}" == "4" || "${LESSON}" == "11" || "${LESSON}" == "25" || "${LESSON}" == "28" ]]; then
    # Whole directories to copy over
    PATHS="
    blockly/media/spritelab
    blockly/media/skins/studio
    blockly/media/common_images
    "
fi
